...
def test_hold_always_allowed(state):
    allowed, _ = guardrails.validate({"action": "hold", "reasoning": "no edge"}, state)
    assert allowed


def test_valid_buy_allowed(state):
    allowed, reason = guardrails.validate(buy(), state)
    assert allowed, reason


def test_oversized_stake_rejected(state):
    allowed, reason = guardrails.validate(buy(stake=guardrails.MAX_STAKE_USDT + 1), state)
    assert not allowed and "stake" in reason


def test_zero_stake_rejected(state):
    allowed, _ = guardrails.validate(buy(stake=0), state)
    assert not allowed


def test_unknown_pair_rejected(state):
    allowed, reason = guardrails.validate(buy(pair="DOGE/USDT"), state)
    assert not allowed and "whitelist" in reason


def test_max_open_trades_rejected(state):
    state.open_trade_count = guardrails.MAX_OPEN_TRADES
    allowed, _ = guardrails.validate(buy(), state)
    assert not allowed
...
def test_sell_of_open_trade_allowed(state):
    allowed, _ = guardrails.validate(
        {"action": "sell", "trade_id": 7, "reasoning": "take profit"}, state)
    assert allowed


def test_sell_of_unknown_trade_rejected(state):
    allowed, _ = guardrails.validate(
        {"action": "sell", "trade_id": 99, "reasoning": "oops"}, state)
    assert not allowed
